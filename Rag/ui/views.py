    ...
    if request.method == "POST":
        ...
        if bool(knolwdge_source):
            ...
            if upload_method != "embeddings":
                ...
                CHUNKS = None
                splitter = SplitData()
                chunk_size_list = request.POST.getlist("chunk_size")
                overlap_list = request.POST.getlist("overlap")
                get_value = Miscellaneous()
                chunk_size = get_value.get_non_empty_value(chunk_size_list)
                overlap = get_value.get_non_empty_value(overlap_list)
                print(overlap, chunk_size)
                print(type(overlap),type(chunk_size))
